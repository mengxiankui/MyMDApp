...
public abstract class MyScrollViewBase<T extends View> extends LinearLayout
{
    ...
    protected DragState mDragState = DragState.NULL;
    ...
    {
        if (isReadyForScrollStart() || isReadyForScrollHeader()||isDuringScrollHeader())
        {
            return true;
        }
        return false;
    ...
    protected abstract boolean isReadyForScrollHeader();
    
    protected abstract boolean isDuringScrollHeader();
        ...
        switch (action)
        {
            case MotionEvent.ACTION_MOVE :
                if (isReadyForScroll() == true)
                {
                    ...
                    else if (isDuringScrollHeader() || (deltaDirection >= 1.0F) && isReadyForScrollHeader())
                    {
                        this.mLastMotionPointF.x = currentX;
                        this.mLastMotionPointF.y = currentY;
                        this.mIsBeingDragged = true;
                        mDragState = DragState.DRAGHEADER;
                    }
                }
                break;
...
//            
    ...
    protected int scrollMoveEvent()
    {
        ...
        if (mDragState == DragState.DRAGSTART)
        {
            scrollLength = Math.min(0, Math.max(0
                - getMaximumScrollOffset(),
                Math.round((from - to) / 2.0F)));
            contentViewScrollTo(scrollLength);
        }
        else if (mDragState == DragState.DRAGHEADER)
        {
            scrollLength = (int) Math.min(
                getHeaderHeight(),
                Math.max(0,
                    Math.round(mScrollContentView.getScrollY() + (from - to))));
            mInitialMotionPointF.x = mLastMotionPointF.x;
            mInitialMotionPointF.y = mLastMotionPointF.y;
            mScrollContentView.scrollTo(0, scrollLength);
        }
        else
        {
            return scrollLength;
        ...
        public void run()
        {
            ...
            else
            {
                ...
                int scrolllength = Math.round((mScrollFromY - mScrollToY)
                    * mInterpolator.getInterpolation((float) scrollDuration / 1000.0F));
